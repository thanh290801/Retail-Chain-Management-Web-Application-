...
const InventoryCheckHistory = () => {
  ...
  useEffect(() => {
    fetch("https://localhost:5000/api/stock-audits/history")
      .then((res) => res.json())
      .then((data) => {
        const sorted = [...data].sort((a, b) => new Date(b.auditDate) - new Date(a.auditDate)); // 👈 Sort mới nhất lên đầu
        setHistory(sorted);
        setFiltered(sorted);
      });
  ...
  useEffect(() => {
    ...
    if (selectedWarehouse !== "all") {
      filteredData = filteredData.filter(
        (h) => h.warehouseId === parseInt(selectedWarehouse)
      );
    }
    if (selectedAuditor !== "all") {
      filteredData = filteredData.filter(
        (h) => h.auditorId === parseInt(selectedAuditor)
      );
    }
    if (searchDate) {
      filteredData = filteredData.filter((h) =>
        h.auditDate?.startsWith(searchDate)
      );
    }
  ...
  const auditorsInWarehouse =
    selectedWarehouse === "all"
      ? employees
      : employees.filter(
          (e) => e.branchId === parseInt(selectedWarehouse)
        );
            ...
            {pageData.length > 0 ? (
              pageData.map((entry, index) => (
                ...
                >
                  ...
                  <td className="p-2 border">
                    {getEmployeeName(entry.auditorId)}
                  </td>
                  <td className="p-2 border">
                    {getEmployeeName(entry.coAuditorId)}
                  </td>
                  <td className="p-2 border">
                    {entry.auditDate.slice(0, 10)}
                  </td>
                </tr>
              ))
            ) : (
          ...
          <div className="mt-4 flex justify-center gap-2">
            ...
            {[...Array(totalPages)].map((_, idx) => (
              <button
                key={idx}
                className={`px-2 ${
                  idx + 1 === currentPage ? "bg-blue-500 text-white" : ""
                }`}
