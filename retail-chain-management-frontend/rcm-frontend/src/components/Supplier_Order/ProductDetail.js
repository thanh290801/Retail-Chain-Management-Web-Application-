...
const ProductDetail = ({ productId: propProductId, isModal }) => {
    const { productId: routeProductId } = useParams();
    const navigate = useNavigate();
    const [product, setProduct] = useState(null);

    const idToUse = propProductId || routeProductId;

    useEffect(() => {
        axios.get(`https://localhost:5000/api/products/${idToUse}`)
            ...
            {!isModal && (
                <>
                    ...
                    <Button
                        ...
                        onClick={() => navigate(`/productedit/${idToUse}`)}
                    >
                        ✏️ Sửa
                    </Button>
                </>
